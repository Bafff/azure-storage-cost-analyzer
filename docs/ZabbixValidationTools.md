...
with open('templates/zabbix-template-azure-storage-monitor-7.0.yaml') as f:
    result = zapi.configuration.import_({
        'format': 'xml',
        'source': f.read(),
...
zabbix-cli --import-template templates/zabbix-template-azure-storage-monitor-7.0.yaml
```
